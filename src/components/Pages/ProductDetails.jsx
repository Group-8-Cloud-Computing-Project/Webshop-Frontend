...
import { Container, Row, Col, Image, Button, Card, Form, Carousel, Modal } from "react-bootstrap";
import { useLocation } from 'react-router-dom';

const ProductDetails = () => {
  const location = useLocation();
  const { product } = location.state;
  const { addToCart, cartError, setCartError } = useAppState();
  ...
  const handleCloseErrorModal = () => { setShowErrorModal(false); setCartError(""); };
  ...
  return (
    <>
      <Modal show={showErrorModal} onHide={handleCloseErrorModal}>
        ...
        <Modal.Body>{cartError}</Modal.Body>
        <Modal.Footer>
